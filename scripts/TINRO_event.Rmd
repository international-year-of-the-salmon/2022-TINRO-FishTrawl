...
tinro_occ <- tinro_catch %>%
  select(eventID = Station_Event_ID,
         ...
         identifiedBy,
         dateIdentified,
         individualCount = Catch_count) # Include individualCount?
...
tinro_samplingInstrument <- merge(tinro_sampling_stations, tinro_samplingInstrument) %>%
  ...
  mutate(measurementTypeID = case_when(
    ...
    measurementType == "Cod-end_Mesh_Size" ~ "http://vocab.nerc.ac.uk/collection/Q01/current/Q0100015/", # Same as above...
...
tinro_trawl2022_samplingEffort$Sampling_Duration_Minutes <- format(tinro_trawl2022_samplingEffort$Sampling_Duration_Minutes, "%H") %>% as.numeric()
tinro_trawl2022_samplingEffort$Sampling_Duration_Minutes <- round(tinro_trawl2022_samplingEffort$Sampling_Duration_Minutes * 60)
...
write_csv(tinro2022_trawl_eMOF_catch, here("standardized_data", "tinro2022_eMOF.csv"))
```
